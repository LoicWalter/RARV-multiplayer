...
public class MovingPlatform : MonoBehaviour
{
    ...
    private void Start()
    {
        // Placer la plateforme au milieu des deux points
        _movingPlatform.transform.position = (_pointA.position + _pointB.position) / 2f;

        targetPosition = _pointB.position; // Initialiser la position cible

        // Gérer la rotation pour qu'elle soit perpendiculaire à la direction du mouvement
        RotatePlatform();
    }
